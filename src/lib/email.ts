...
export async function sendLeadConfirmationEmail(payload: LeadEmailPayload): Promise<EmailResult> {
  const transporter = getTransporter();
  if (!transporter) {
    return { sent: false, skipped: true, reason: "SMTP not configured" };
  }
  ...
  const from = process.env.SMTP_FROM || process.env.SMTP_USER || ENTRAIOT_CONTACT.infoEmail;
  const subject = "Welcome to Entraiot! Your AI Strategy Plan is on the way.";

  const text = [
    `Hi ${payload.name || "there"},`,
    "",
    "Thanks for reaching out to Entraiot! 🎉",
    "",
    "We've received your inquiry regarding " + (payload.intent || "AI solutions") + ".",
    "Our team is already working on your custom AI strategy plan to help your business grow.",
    "",
    "One of our experts will contact you within the next 2 hours to discuss how we can specifically help you:",
    payload.intent === "LEADS" ? "• Generate more leads automatically" : "",
    payload.intent === "AUTOMATION" ? "• Automate your business processes" : "",
    payload.intent === "COST" ? "• Reduce operational costs by 20–40%" : "",
    "",
  ...
  try {
    ...
    console.error("sendLeadConfirmationEmail error:", err);
    return { sent: false, skipped: false, reason: String(err) };
  }
}
