            ...
            <div className="space-y-6 md:space-y-8">
                <div className="space-y-6">
                    <div className="mb-6 text-left">
                        ...
                        <h2 className="dark-readable text-3xl md:text-4xl font-[800] text-slate-900 mb-4">
                    ...
                    <GlowEffect color="#2563eb" intensity={0}>
                        <div className="relative group bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 border-l-[4px] border-l-[#2563eb] rounded-[16px] p-6 shadow-sm backdrop-blur-sm">
                            <div className="flex justify-between items-center mb-3">
                                <span className="text-xs font-bold uppercase tracking-widest text-[#2563eb]">OUR VISION</span>
                                <Target className="w-5 h-5 text-[#2563eb]" />
                            </div>
                            <p className="dark-muted-readable text-base text-slate-800 dark:text-slate-200 font-medium leading-[1.7] opacity-100">
                    ...
                    <GlowEffect color="#7c3aed" intensity={0}>
                        <div className="relative group bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 border-l-[4px] border-l-[#7c3aed] rounded-[16px] p-6 shadow-sm backdrop-blur-sm">
                            <div className="flex justify-between items-center mb-3">
                        ...
                        <div className="flex flex-col gap-3">
                            {visionPillars.slice(2, 4).map((pillar, i) => {
                                ...
                                return (
                                    <div 
                                        key={actualIndex} 
                                        className={`rounded-[16px] overflow-hidden transition-all duration-350 border-l-[3px] bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 ${isActive ? 'shadow-md' : ''}`}
                                        style={{ 
                                    ...
                                    >
                                        <button
                                            ...
                                            className={`w-full flex items-center justify-between p-4 transition-all ${isActive ? 'bg-slate-50 dark:bg-slate-800' : 'hover:bg-slate-50/50 dark:hover:bg-slate-800/70'}`}
                                        >
                                          <div className="flex items-center gap-3">
                                            ...
                                            <span className={`dark-readable text-sm font-bold transition-colors ${isActive ? 'text-slate-900 dark:text-slate-50' : 'text-slate-700 dark:text-slate-200'}`}>{pillar.title}</span>
                                          </div>
                                        ...
                                        >
                                            <div className="p-5 pt-3 bg-white dark:bg-slate-900 border-t border-slate-100 dark:border-slate-700">
                                                <p className="dark-muted-readable text-sm text-slate-700 dark:text-slate-200 font-normal leading-[1.7]">{pillar.text}</p>
                                                <div className="flex flex-wrap gap-2 mt-4">
                                ...
                                <GlowEffect color="#6366f1" intensity={0}>
                                    <div className="glass backdrop-blur-sm rounded-2xl p-4 border border-border/20 shadow-lg cursor-pointer transition-all duration-200 ease-out hover:scale-105 hover:-translate-y-2">
                                        <div className="text-center space-y-2">
                                            <h4 className="dark-readable font-bold text-foreground text-lg">Salvin Jones</h4>
                ...
                <div className="mt-12 lg:hidden space-y-4">
                    {founderAchievements.map((achievement, idx) => (
                        <motion.div
                            ...
                            className="p-4 rounded-2xl bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 shadow-sm flex gap-4 items-start"
                        >
                            <div className="w-10 h-10 rounded-xl bg-blue-50 flex items-center justify-center text-xl shrink-0">
                                {achievement.icon}
                            </div>
                            <div>
                                                <h5 className="dark-readable font-bold text-slate-900 text-sm mb-1">{achievement.title}</h5>
                                                <p className="dark-muted-readable text-xs text-slate-700 leading-relaxed">
                                    {achievement.description}
                                </p>
                            </div>
                        </motion.div>
                        ...
                        <GlowEffect color="#6366f1" intensity={0}>
                            <div className="text-center bg-white/95 dark:bg-white/15 backdrop-blur-sm rounded-lg sm:rounded-xl p-2.5 sm:p-3 border border-border/20 shadow-sm cursor-pointer transition-all duration-200 hover:shadow-md hover:scale-105">
                                <div className="text-base sm:text-lg font-bold text-blue-700 dark:text-blue-400">
                                    <ScrollCounter from={0} to={3500} suffix="+" duration={3.5} className="tabular-nums" />
                                </div>
